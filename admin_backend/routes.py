    ...
    try:
        ...
        if response.user:
            ...
            supabase.table("users").insert(
                {"email": email, "role": "admin", "auth_user_id": response.user.id}
            ).execute()
...
def login():
    ...
    try:
        ...
        if response.user and response.session:
            ...
            user_data = (
                supabase.table("users").select("role").eq("email", email).execute()
            )
...
def reset_system():
    try:
        ...
        supabase.table("parking_spots").update({"is_occupied": False}).neq(
            "id", 0
        ).execute()
...
def vehicle_entry():
    ...
    if active_result.data:
        ...
        return (
            jsonify(
                {
                    "message": f'Vehicle {plate_number} is already parked at {session["spot_name"]}'
                }
            ),
            409,
        )
    ...
    if free_spot_result.data:
        ...
        supabase.table("parking_spots").update({"is_occupied": True}).eq(
            "id", free_spot["id"]
        ).execute()
...
def vehicle_exit():
    ...
    supabase.table("parking_spots").update({"is_occupied": False}).eq(
...
def add_camera():
    ...
    existing = (
        supabase.table("cameras").select("id").eq("camera_id", camera_id).execute()
    )
    if existing.data:
    ...
    return (
        jsonify(
            {"message": f"Camera {name} added successfully", "id": result.data[0]["id"]}
        ),
        201,
    )
